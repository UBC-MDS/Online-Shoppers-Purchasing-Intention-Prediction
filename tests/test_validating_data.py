import pytest
import os
import numpy as np
import pandas as pd
import pandera as pa
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from src.validating_data import validating_data

def test_valid_dataframe():
    """Test a valid DataFrame passes validation."""
    valid_data = {
        "Administrative": [1, 2],
        "Administrative_Duration": [100.0, 200.0],
        "Informational": [0, 1],
        "Informational_Duration": [50.0, 150.0],
        "ProductRelated": [10, 20],
        "ProductRelated_Duration": [300.0, 600.0],
        "BounceRates": [0.1, 0.2],
        "ExitRates": [0.3, 0.4],
        "PageValues": [0.0, 100.0],
        "SpecialDay": [0.0, 0.8],
        "Month": ["Jan", "Feb"],
        "OperatingSystems": [1, 2],
        "Browser": [1, 2],
        "Region": [1, 2],
        "TrafficType": [1, 2],
        "VisitorType": ["New_Visitor", "Returning_Visitor"],
        "Weekend": [True, False],
        "Revenue": [False, True],
    }
    df = pd.DataFrame(valid_data)
    # No exception should be raised
    validating_data(df)

def test_invalid_type():
    """Test input is not a pandas DataFrame."""
    with pytest.raises(TypeError, match="Input must be in data type: a pandas DataFrame"):
        validating_data([])

def test_empty_dataframe():
    """Test an empty DataFrame."""
    empty_df = pd.DataFrame()
    with pytest.raises(ValueError, match="Dataframe cannot be empty, i.e. no rows."):
        validating_data(empty_df)

def test_missing_columns():
    """Test DataFrame with missing required columns."""
    invalid_df = pd.DataFrame({
        "Administrative": [1],
        "Administrative_Duration": [100.0]
    })
    with pytest.raises(pa_errors.SchemaError):
        validating_data(invalid_df)

def test_invalid_data_types():
    """Test DataFrame with invalid column data types."""
    invalid_df = pd.DataFrame({
        "Administrative": ["1"],
        "Administrative_Duration": [100.0],
        "Informational": [0],
        "Informational_Duration": [50.0],
        "ProductRelated": [10],
        "ProductRelated_Duration": [300.0],
        "BounceRates": [0.1],
        "ExitRates": [0.3],
        "PageValues": [0.0],
        "SpecialDay": [0.0],
        "Month": ["Jan"],
        "OperatingSystems": [1],
        "Browser": [1],
        "Region": [1],
        "TrafficType": [1],
        "VisitorType": ["New_Visitor"],
        "Weekend": [True],
        "Revenue": [False],
    })
    with pytest.raises(pa_errors.SchemaError):
        validating_data(invalid_df)

def test_out_of_range_values():
    """Test DataFrame with out-of-range values."""
    invalid_df = pd.DataFrame({
        "Administrative": [1],
        "Administrative_Duration": [90000.0],  # Invalid: exceeds range
        "Informational": [0],
        "Informational_Duration": [50.0],
        "ProductRelated": [10],
        "ProductRelated_Duration": [300.0],
        "BounceRates": [1.2],  # Invalid: exceeds 1
        "ExitRates": [0.3],
        "PageValues": [0.0],
        "SpecialDay": [0.0],
        "Month": ["Jan"],
        "OperatingSystems": [1],
        "Browser": [1],
        "Region": [1],
        "TrafficType": [1],
        "VisitorType": ["New_Visitor"],
        "Weekend": [True],
        "Revenue": [False],
    })
    with pytest.raises(pa_errors.SchemaError):
        validating_data(invalid_df)

def test_invalid_categorical_values():
    """Test DataFrame with invalid categorical values."""
    invalid_df = pd.DataFrame({
        "Administrative": [1],
        "Administrative_Duration": [100.0],
        "Informational": [0],
        "Informational_Duration": [50.0],
        "ProductRelated": [10],
        "ProductRelated_Duration": [300.0],
        "BounceRates": [0.1],
        "ExitRates": [0.3],
        "PageValues": [0.0],
        "SpecialDay": [0.0],
        "Month": ["InvalidMonth"],  # Invalid month
        "OperatingSystems": [1],
        "Browser": [1],
        "Region": [1],
        "TrafficType": [1],
        "VisitorType": ["InvalidVisitor"],  # Invalid visitor type
        "Weekend": [True],
        "Revenue": [False],
    })
    with pytest.raises(pa_errors.SchemaError):
        validating_data(invalid_df)
        
# Parameterize invalid data test cases
@pytest.mark.parametrize("invalid_data, description", invalid_data_cases)
def test_valid_w_invalid_data(invalid_data, description):
    with pytest.raises(pa.errors.SchemaErrors) as:
        validate_data(invalid_data)