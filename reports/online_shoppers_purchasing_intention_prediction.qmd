...
To investigate the features in our dataset, we first visualized the correlation between each pair of features using a heatmap. From this, we can see that feautures are not too correlated with each other, and strong correlations only appear when a feature is compared to itself.
...
From cross-validation , we found that our best logistic regression model with C = `{python} best_C` yielded an validation accuracy score of `{python} logreg_cv_score`%, which is slightly better (`{python} round(logreg_cv_score-dummy_cv_score,1)`%) compared to the validation accuracy score of our dummy classifier using a most-frequent strategy (`{python} dummy_cv_score`%).[@tbl-validation_scores]
...
After testing our model using the testing data, we found that our best logistic regression model with C = `{python} best_C` had a test accuracy score of `{python} logreg_test_score`%, which is again a bit better (`{python} round(logreg_test_score-dummy_test_score,1)`%) that the test accuracy score of our dummy classifier using a most-frequent strategy (`{python} dummy_test_score`%). [@tbl-test_scores]
...
![Model features and their associated absolute value weight.](../results/images/fig4_feat_weights.png){#fig-feature_bar_plot width=85%}
